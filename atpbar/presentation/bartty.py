...
class ProgressBar(Presentation):
    ...
    def _compose_just_finised_bars(self):
        self.just_finised_bars = [self._compose_bar_from_taskid(i) for i in self._finishing_taskids]

    def _compose_active_bars(self):
        self.active_bars = [self._compose_bar_from_taskid(i) for i in self._active_taskids + self._new_taskids]
    ...
    def _compose_bar_from_report(self, report):
        ...
        ret = format.format(percent=percent, bar=bar, done=report["done"], total=report["total"], name=report["name"])
